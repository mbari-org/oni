...
object Scripts {
  ...
  /**
   * Generate a temporary file that concatenates all the SQL files in the given resource
   * path
   * @param srcPath the resource path to the SQL files, e.g "/sql". This is not
   *                a file path but a path to a directory in the resources of the
   *                java classpath
  ...
  /**
   * Run the SQL script at the given path. It is assumed that the script contains
   * multiple SQL statements separated by a semicolon.
