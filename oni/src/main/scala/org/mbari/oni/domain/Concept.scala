 ...
 *
 * @param name
 *   The primary, or accepted, name of the concept
 * @param rank
 *   The pyhologenetic rank
 * @param alternativeNames
 *   Synonyms, common names, or former names
 * @param children
 *   Child concepts of this concept
 * @param aphiaId
 *   The WoRMS AphiaID
 * @param id
 *   The database id
...
case class Concept(
     ...
     *   The name to check
     * @return
     *   true if this concept's name or alternative names contain this name. false if it does not.
     */
    def containsName(n: String): Boolean = name.equals(n) ||
        alternativeNames.contains(n)

    /**
     * All names applicable to this concept. THe first name will be the primary name.
     */
    lazy val names: Seq[String] = name +: alternativeNames

    /**
     * A sorted list all names applicable to this concept and all of it's descendants.
     */
