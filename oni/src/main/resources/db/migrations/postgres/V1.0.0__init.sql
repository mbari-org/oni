...
create table Concept (AphiaId bigint, LAST_UPDATED_TIME timestamp(6), ParentConceptID_FK bigint, id bigint not null, RankLevel varchar(20), RankName varchar(20), primary key (id));
create table ConceptDelegate (ConceptID_FK bigint not null unique, LAST_UPDATED_TIME timestamp(6), id bigint not null, primary key (id));
create table ConceptName (ConceptID_FK bigint not null, LAST_UPDATED_TIME timestamp(6), id bigint not null, NameType varchar(10) not null, ConceptName varchar(128) not null unique, Author varchar(255), primary key (id));
create table History (Approved smallint, ConceptDelegateID_FK bigint not null, CreationDTG timestamp(6) not null, LAST_UPDATED_TIME timestamp(6), ProcessedDTG timestamp(6), id bigint not null, Action varchar(16), CreatorName varchar(50) not null, ProcessorName varchar(50), Field varchar(2048), NewValue varchar(2048), OldValue varchar(2048), primary key (id));
create table LinkRealization (ConceptDelegateID_FK bigint not null, LAST_UPDATED_TIME timestamp(6), id bigint not null, LinkName varchar(50), ToConcept varchar(128), LinkValue varchar(2048), primary key (id));
create table LinkTemplate (ConceptDelegateID_FK bigint not null, LAST_UPDATED_TIME timestamp(6), id bigint not null, LinkName varchar(50), ToConcept varchar(128), LinkValue varchar(2048), primary key (id));
create table Media (PrimaryMedia smallint, MediaType varchar(5), ConceptDelegateID_FK bigint not null, LAST_UPDATED_TIME timestamp(6), id bigint not null, Caption varchar(1000), Url varchar(1024), Credit varchar(255), primary key (id));
...
create table UserAccount (LAST_UPDATED_TIME timestamp(6), id bigint not null, Role varchar(10) not null, Email varchar(50), FirstName varchar(50), LastName varchar(50), Password varchar(50) not null, UserName varchar(50) not null unique, Affiliation varchar(512), primary key (id));
...
alter table if exists Concept add constraint fk_Concept__Concept_id foreign key (ParentConceptID_FK) references Concept;
alter table if exists ConceptDelegate add constraint fk_ConceptDelegage__Concept_id foreign key (ConceptID_FK) references Concept;
alter table if exists ConceptName add constraint fk_ConceptName__Concept_id foreign key (ConceptID_FK) references Concept;
alter table if exists History add constraint fk_History__ConceptDelegate_id foreign key (ConceptDelegateID_FK) references ConceptDelegate;
